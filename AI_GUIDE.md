...
**※ 適用除外（重要）：** §12 の 3Dplus 空間ツリー（`node.local` / `node.world` 等）には、この規約を適用しません。なぜなら、3D空間演算では「衝突判定」や「マウスピッキング（IK等）」など、World（結果）からLocal（真実）へ逆算して値を補正する**双方向のフィードバック**が不可避に発生するためです。完全一方向を強制する REAL / SHADOW 規約を3Dに持ち込むと破綻します。空間計算は完全に §12 の管轄とします。
...
// 状態の更新：真実を直接書き換える
function equipWeapon(newWeaponAtk) {
  REAL_playerData.weaponAtk = newWeaponAtk;
}
```

この規約により、「状態Aは更新したが、そこから計算される変数Bを更新し忘れた」というAI特有の同期漏れバグを物理的に排除します。
// [/ai_s_emblem: State-Architecture]
