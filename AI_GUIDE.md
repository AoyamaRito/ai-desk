...
### 12.2 3Dplus の定義
座標とは空間だけではありません。「いつ、どれくらい、存在するか」を含む以下の6要素を「座標」として扱い、親から子へ一方向に投影します。
- **`x, y, angle`**: 空間的位置・回転
...
### 12.4 AIの責務
- **Logic（意図）**: AIは `node.local` のみを書き換えます。Worldを直接触ることは「禁忌」です。
- **Draw/Sensor（実行）**: AIは `node.world` のみを参照します。Localを見て計算することは「無駄」です。
- **時空操作**: スローモーション、フェードアウト、親子連動は、すべて「親のLocal」をいじるだけで完結させます。
