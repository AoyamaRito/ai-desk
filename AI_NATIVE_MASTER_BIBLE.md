...
## 0.1 ローカリティの極大化 (Locality Maximization)
AI向けコードにおいて、ローカリティは最優先の設計原則である。

ファイル分割は情報を物理的に散らす。
関数分割は情報を論理的に散らす。
どちらもAIに対して、参照・探索・推測のコストを発生させる。

したがって、AI向けコードの設計では、処理の意味・前提・分岐・副作用を、可能な限り同一スコープ内に集約する。論理層（JS）は必ず1ファイルに集約し、`ai-desk` の Emblem によって仮想的な認知境界を運用する。

大きな関数は、必ずしも悪ではない。依存関係を持たない大きなコードは、AIにとってはただの長さにすぎない。複雑さの本質はサイズではなく依存グラフの密度である。

小さな関数は、一見整理されているように見える。しかし、小さな関数は複数箇所から呼ばれることで共有状態となり、変更の影響範囲が不可視になる。一箇所の修正が、把握していない呼び出し元を壊す。AIにとっては「この関数を変更して安全か」の判断に、全呼び出し元の探索が必要になる。

AI-Native 開発においては、抽象化よりも局所完結性を優先する。
再利用性よりも変更安全性を優先する。
美しい分割よりも、迷わない配置を優先する。
