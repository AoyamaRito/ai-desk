...
> このファイルは **AI / アーキテクト向け**の全体仕様・設計思想・アーキテクチャ解説です。
> 一般ユーザは先に [`README.md`](README.md) を読んでください (自然言語で AI に作業を頼む方法)。
> AI は作業を始める前に [`AI_ONBOARDING.md`](AI_ONBOARDING.md) も合わせて読んでください。

---

## ブランチ運用

- **`main`** — 開発主幹。最新の commit がここに入る。デフォルトブランチ。
- **`stable`** — ピン留め安定版。**main から手動で fast-forward** したときだけ前進する (毎 commit 同期はしない)。外部統合・citation で「ai-desk@stable 時点」と参照したい人向け。
...
「これはバグでは？」と思ったら: このコードは意図的に同じ regex を複数箇所に複製し、関数を長く保っています。設計の「なぜ」は本ファイル以下と `AI_NATIVE_MASTER_BIBLE.md` にあります。
...
- **アテンションが分散し、ひとつの目的に束ねられない形のTDD** — テストとコードがファイル分離される、またはサイクル進行中に「達成すべき仕様の全体像」が AI に見えない構造のテスト駆動は、AI の迷走を生む（§1 スポットライト + §3 切替コスト + §4 情報欠落の三重違反）。AI は目先のテスト1件を通すだけの場当たりコードを書き、後続のテストで矛盾が露呈してリファクタリング地獄に陥る。
  ...
  否定対象は「テスト」ではなく **「目的の分散」** である。テスト自体は Bible §7（複式数学）が要求している。**一つの目的に束ねられ、AI が full context を一望できるテスト群**（例: 本リポの [`fighter-cancel.test.js`](demos-legacy-v1/actions/tests/fighter-cancel.test.js) — 19件の網羅テストが1ファイルに集約され、対応するロジックも単一スコープに収まる）は、むしろ AI ネイティブ最適の検証手段となる。
...
- **網羅型テスト群** — 「全可能世界を列挙して矛盾を検査する」型のテスト（例: [`fighter-cancel.test.js`](demos-legacy-v1/actions/tests/fighter-cancel.test.js)、1920世界を1ファイルで網羅）。
...
3. **条件畳み込み（Constraint Folding）** — if/elseの代わりに「全可能世界 → 制約filter」で結果を導出（[Bible §0.15](AI_NATIVE_MASTER_BIBLE.md)、[実装ガイド](CONSTRAINT_FOLDING_MASTER.md)）
4. **鉱脈採掘パラダイム** — モンテカルロ + LLMによる法則解読 + 制約バリデーターの三段検証（[CONSTRAINT_FOLDING_MASTER.md](CONSTRAINT_FOLDING_MASTER.md) 内に統合）
...
> **🎮 [Playable Demo (Action Constraint Lab) をブラウザで開く](https://aoyamarito.github.io/ai-desk/demos-legacy-v1/actions/index.html)**
...
| [`CONSTRAINT_FOLDING_MASTER.md`](CONSTRAINT_FOLDING_MASTER.md) | 制約畳み込みパターン LLM 適用ガイド + 鉱脈採掘パラダイム解説（送料計算 PoC 含む） |
| [`demos-legacy-v1/actions/ACTION_NATIVE_FOLDING_GUIDE.md`](demos-legacy-v1/actions/ACTION_NATIVE_FOLDING_GUIDE.md) | アクションゲームへの適用ガイド（三層畳み込みアーキテクチャ） |
| [`demos-legacy-v1/3dplus/README.md`](demos-legacy-v1/3dplus/README.md) | 複式数学の3D実装ガイド。GPUと並走するCPU側検証層（cpu3d.js）の契約と使い方 |
...
### アクションゲーム適用
- [**`action-demos.html`**](https://aoyamarito.github.io/ai-desk/demos-legacy-v1/action-demos.html) + `action-demos.js` — 9 アクションゲームアルゴリズム（プレイアブル）
- [**`demos-legacy-v1/actions/index.html`**](https://aoyamarito.github.io/ai-desk/demos-legacy-v1/actions/index.html) — `fighter-cancel` 実装＋19/19網羅テスト

### 鉱脈採掘 PoC
- [`demos-legacy-v1/examples/`](demos-legacy-v1/examples/) — レガシー送料計算ロジックを 50 サンプルから 100% 復元した実証実験

### 複式数学 / 3Dplus 検証層
- [`demos-legacy-v1/3dplus/cpu3d.js`](demos-legacy-v1/3dplus/cpu3d.js) + [`demos-legacy-v1/3dplus/render.js`](demos-legacy-v1/3dplus/render.js) — CPU Twin と GPU レンダラーのペア（同一 scene JSON フォーマット）
...
open demos-legacy-v1/3dplus/examples/point-projection.html  # GPU vs CPU 突合デモ
```
...
```
ai-desk/
├── README.md                      # 一般ユーザ向けクイックスタート
├── README.ai.md                   # このファイル (AI / アーキテクト向け詳細)
...
├── CONSTRAINT_FOLDING_MASTER.md    # 制約畳み込みガイド + 鉱脈採掘パラダイム
