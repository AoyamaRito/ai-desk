...
```bash
# fighter-cancel: 制約畳み込みで格闘ゲームのキャンセルチェーンを実装
...
- **`ai-eyes.js` (目)** — ブラウザエラーの自動収集、HTML/Canvasスナップショットの保存、およびAIからのリモート操作（Step-by-step デバッグ）を受け付けるゼロ依存ローカルサーバー

```bash
node ai-eyes.js  # サーバー起動 (http://localhost:3000)

# AI は以下のコマンドで自律的にデバッグを行う
...
### アクションゲーム適用
- [`action-demos.html`](action-demos.html) + [`action-demos.js`](action-demos.js) — 9 アクションゲームアルゴリズム（プレイアブル）
- [`actions/`](actions/) — 単一HTMLハブから複数JSデモを切替えるラボ。`fighter-cancel` 実装＋19/19網羅テスト
...
- [`3dplus/tests/projection.test.js`](3dplus/tests/projection.test.js) — 行列・階層・3Dplus軸の網羅検証 21/21 PASS
