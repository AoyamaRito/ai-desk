...
// フレームはメモリ上に base64 文字列として保持し、停止時に HTML スライドショーへ出力。
// ffmpeg 依存なし。Zero-Dep 完全準拠。
...
function saveFrame(base64Image) {
  ...
  recordedFrames.push(base64Image); // data:image/png;base64,... のまま保持
  if (recordedFrames.length % 10 === 0) console.log(`[Video] Captured frame ${recordedFrames.length} for session ${currentRecordingId}`);
}

function stopRecording() {
  if (!currentRecordingId || recordedFrames.length === 0) {
  ...
  const outputFilename = `video_${timestamp}.html`;
...
<head>
  <meta charset="UTF-8">
  <title>Recording ${timestamp} (${frameCount} frames @ 10fps)</title>
  <style>
    body { margin: 0; background: #000; display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100vh; color: #fff; font-family: monospace; }
    img { max-width: 100%; max-height: 85vh; }
    #info { margin-top: 8px; font-size: 12px; opacity: .6; }
    #controls { margin-top: 6px; display: flex; gap: 8px; }
...
<body>
  ...
  <div id="controls">
    ...
    <input id="scrub" type="range" min="0" max="${frameCount - 1}" value="0" style="width:200px">
  </div>
  <script>
    const frames = ${JSON.stringify(recordedFrames)};
    ...
    function show(n) {
      img.src = frames[n];
      info.textContent = (n + 1) + ' / ' + frames.length;
      scrub.value = n;
    }
    ...
    scrub.oninput = () => { i = parseInt(scrub.value); show(i); };
    show(0);
  </script>
</body>
</html>`;
  ...
  console.log(`[Video] HTML slideshow saved: ${outputFilename} (${frameCount} frames)`);

  currentRecordingId = null;
  recordedFrames.length = 0;
  return { ok: true, file: outputFilename };
}
...
(function() {
  ...
  // --- Recording Endpoints ---
    ...
    const baseDir = path.resolve(SNAPSHOT_DIR);
    const filepath = path.resolve(baseDir, file);
    if (filepath.startsWith(baseDir + path.sep) && fs.existsSync(filepath) && file.endsWith('.html')) {
      res.writeHead(200, { 'Content-Type': 'text/html' });
      res.end(fs.readFileSync(filepath));
    } else {
      res.writeHead(404); res.end('Not Found');
...
function testCleanup() {
  ...
  if (fs.existsSync(TEST_SNAPSHOTS)) fs.rmSync(TEST_SNAPSHOTS, { recursive: true, force: true });
}
...
async function runE2ETests() {
  ...
  try {
    ...
    // Test VideoManager (sync rewrite: Promise → direct return)
    console.log('\n[Test: VideoManager]');
    res = await testFetch('/record/start', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' });
    assert(res.status === 200, 'POST /record/start Status 200');
    data = JSON.parse(res.data);
    assert(typeof data.id === 'string', 'Returns recording id');
    ...
    res = await testFetch('/frame', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image: dummyFrame }) });
    assert(res.status === 200, 'POST /frame Status 200');

    res = await testFetch('/record/stop', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' });
    assert(res.status === 200, 'POST /record/stop Status 200');
    data = JSON.parse(res.data);
    assert(data.ok === true, 'stopRecording ok:true');
    assert(typeof data.file === 'string' && data.file.endsWith('.html'), 'Output is .html (not .mp4)');

    res = await testFetch('/videos/' + data.file);
    assert(res.status === 200, 'GET /videos/{file} Status 200');
    assert(res.data.includes('<img id="frame"'), 'HTML contains slideshow img element');

    // no-frames guard
    res = await testFetch('/record/stop', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' });
    data = JSON.parse(res.data);
    assert(data.ok === false, 'stopRecording with no frames: ok:false');
...
WARNING: 開発専用ツール。localhost のみで使用すること。
         /input エンドポイントはブラウザ上で任意コードを eval する。
         公開ネットワークには絶対に晒すな。
