# 3dplus (v2 ported) — CPU 3D Twin

> **GPU を信じるな。検算せよ。**
> v1 から **無改造で v2 に移植**した CPU 側の透明 3D 算数(2026-05-03)。
> v1 emblem マーカー(`// [ai_s_emblem:...]`)のみ剥離、コードと振る舞いは一切変更なし。
> 81/81 tests green(animation 10 + collision 11 + ik 4 + projection 55 + skinning 1)。

## v2 における位置づけ

このモジュールは v2 の公理 A0(展開された複雑性)・A7(LLM-First 情報密度)を 3D に持ち込むための**標準資産**として保存。
3D を扱う v2 demo / 内部ロジック(将来の fighter-cancel / swarm / space-race / spec を伴う物理シミュレーション等)が、GPU や外部物理エンジンを使うとき、この CPU twin と突合してバグの所在を**断定可能**にする。

**voxel demo (2026-05-03 試行) では効果を出せなかった**理由: 純 CPU → canvas 2D の単一パイプラインだったため、Twin になる相手(GPU 出力)がいなかった。3dplus 本領は**「GPU vs CPU の突合」**にある。

## v1 → v2 の差分

- `// [ai_s_emblem:...]` マーカーを全削除(v2 §4「マーカー廃止」)
- `package.json { "type": "commonjs" }` を追加(v2 root の `"type": "module"` に対し、3dplus は CommonJS のまま動かすため局所オーバーライド)
- 内部 import / export(`require` / `module.exports`)はそのまま、Bible §4.5 の Twin 規約への参照記述もそのまま

## 現状の使われ方

- 単体: 81/81 tests green、`node tests/<name>.test.js` で実行可能
- v2 demos からの呼び出し: 現状なし(将来 GPU を使う demo が出てきたときに突合層として導入)

---

(以下は v1 時代の README 本文を保持)
