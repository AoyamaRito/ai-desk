...
## ⚡ 大前提 ─ 複雑性の重力性(全公理の前に内面化すべきこと)

ai-desk のすべての設計は、以下の物理原理から演繹される:

> **複雑性は、LLM の推論軌道を一意化する「重力場」である。**
> シンプル = 重力ゼロ空間 = ハルシネーション温床。
> 複雑性 = 重力場 = 推論軌道の必然化。
> 隠匿(=情報の欠落)は推測領域を発生させ、ハルシネーションを誘発する「毒」である。

この原理を内面化せずに以下の公理 A0〜A7 を読むと、
各公理が単なる「変わった主張」に見えるだけで、**体系として理解できない**。

人間直観(シンプル = 安心、複雑 = 混乱)は人間の作業記憶容量(7±2)による錯覚であり、
LLM の作業記憶容量では逆になる(複雑 = 安定、シンプル = 漂流)。

ai-desk は「LLM の重力場を意図的に構築する」設計体系である。
詳細解説は §2.5「複雑性の重力性」を参照。
...
- **§0.19 Spec-First Versioning (脱皮プロトコル)** — 仕様を Block の履歴の起点(v0)およびマイルストーンとして物理的に埋め込む。
...
### 公理 A8(新規) — Spec-First Versioning (論理の脱皮)

仕様（What）と実装（How）を同一の履歴（versions）の中に、交互のレイヤーとして積み重ねる。

1. **v0 = 論理（Spec）**: すべての Block の最初のバージョンは、コードを書かず、コメントによる仕様記述のみとする。
2. **v1 = 肉体（Impl）**: v0 の論理に基づき、最初の手続きを実装する。
3. **脱皮プロトコル**: 仕様を変更する場合、既存のコードに直接手を入れるのではなく、一度「新仕様のみ（コードなし）」のバージョンを挟み、論理を確定させてから次のバージョンで新しい肉体（実装）を宿す。
4. **構造的 enforcement**: 上記の規律を強制するための lint 関数や validate 関数は**書かない**。`#SPEC#` タグを持つ version の存在/不在、および versions の prevHash チェーンの構造そのものが enforcement になる(§4.1.1)。検証関数を別途用意することは A5(All-as-Block)を崩すため禁忌。

これは、実装の複雑化に伴う**文脈の漂流（Context Drift）**を物理的に防止し、AI がいつでも原初の、あるいは最新の「正解」へ帰還できるようにするための**重力アンカー**である。
...
### 4.1 #SPEC# タグによる論理のマーク

Spec-First Versioning を機械的に識別するため、仕様のみのバージョンには **`#SPEC#`** タグを付与する。

- **役割**: `versions` の中から「最後に `#SPEC#` が付与されたバージョン」を探すだけで、現在の論理的な正解（仕様）を特定できる。
- **検索性**: `ai-desk tags SPEC` により、プロジェクト全体の「論理設計の変遷」を瞬時に抽出可能。

### 4.1.1 構造的 enforcement(検証関数を書かない)

公理 A8 の規律(spec-first / 脱皮プロトコル)は、**ai-desk.js に lint や validate 関数を追加する形では実装しない**。
代わりに、versions の構造そのものから読む。

| 検査項目 | 構造から読む方法 |
|---|---|
| Block が spec を持っているか | `tags SPEC` の出力にその Block が現れるか |
| 最新仕様 | `versions` 内で最後に `#SPEC#` を持つ version |
| 仕様を挟まず impl 直書きしていないか | 直近 impl version の prevHash チェーンを遡ったとき、impl 同士が連続している(間に `#SPEC#` 版が無い)= spec 変更を挟まなかった証拠 |
| spec と impl の整合 | 最新 `#SPEC#` 以降にコミットされた impl version の数で「未着手仕様」が即わかる |

これらは**全て既存の Block 構造を読むだけ**で導出できる。
専用の lint 関数を増やすことは A5(All-as-Block)に反する — 構造で読めるものを関数として再実装すれば、それは新しい型を増やしているのと同じ毒(§10-1)。

**原則**: ai-desk の規律は、規律を強制する関数ではなく、**規律違反が自動的に可視化される構造**で実装する。
...
**Date**: 2026-05-03
**Author**: 沖井広行(蒼山りと)
**Implementation**: `ai-desk.js` (~1.7k 行、Zero-Dep、105 e2e tests passing)
